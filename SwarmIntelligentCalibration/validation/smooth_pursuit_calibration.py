...
if __name__ == '__main__':
    ...
    for sub_item in np.unique(id_map.subj.values):
        ...
        train_dataset = select_data_by_id(smooth_pursuit_data, smooth_pursuit_id[0])

        for r_id in random_id:
            test_dataset = select_data_by_id(random_data, r_id)
            res = calibration(train_dataset, test_dataset, regression_type=regression_type)
            random_result.append(res)

        for z_id in zigzag_id:
            test_dataset = select_data_by_id(zigzag_data, z_id)
            res = calibration(train_dataset, test_dataset, regression_type=regression_type)
            zigzag_result.append(res)

    data_type = "results/smooth_pursuit"
    if not os.path.exists(data_type):
        os.makedirs(data_type)

    np.save(os.path.join(data_type, data_source) + "_" + regression_type + "_" + "random", random_result)
    np.save(os.path.join(data_type, data_source) + "_" + regression_type + "_" + "zigzag", zigzag_result)
