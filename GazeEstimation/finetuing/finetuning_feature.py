...
def do_evaluation(net, test_loader, unique_id, iTracker_format=False):
    net.eval()
    count = 0
    total = 0
    print("Testing")
    ...
    with torch.no_grad():
        ...
        # print(buffer_list.shape)
        # print(feature_path)
        if not os.path.exists(unique_id):
            os.makedirs(unique_id)

        np.savez_compressed(os.path.join(unique_id, f"Iter_6_feature"), buffer_list)
...
def runSession(train_loader, test_loader, unique_id):
    ...
    for epoch in range(1, n_epoch + 1):
        ...
        for i, (data) in enumerate(train_loader):
            ...
            sys.stdout.flush()
        if epoch == 6:    
            do_evaluation(net, test_loader, unique_id)
            break
    #   np.save(os.path.join(result_dir, unique_id), result_)


if __name__ == '__main__':
    our_lissajous_data_path = ["/media/robotics/gazeCapture-202211/lissajous"]
    our_smooth_data_path = ["/media/robotics/gazeCapture-202211/smooth"]
    our_random_data_path = ["/media/robotics/gazeCapture-202211/random"]
    ...
    result_dir = f"result_finetuning/{args.data_source}"
    ...
    for sub_item in np.unique(id_map.subj.values):
        ...
        # unSession(random_dataset, smooth_pursuit_dataset, "%s_random_zigzag" % sub_item)
